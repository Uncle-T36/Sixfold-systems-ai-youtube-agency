...
import { analyzeVideoByCouncil, type VideoCouncilAnalysis } from './imperialCouncil';
...
export interface PlannedVideo {
  ...
  councilAnalysis?: VideoCouncilAnalysis; // Imperial Council approval
...
export async function autoGenerateFirstVideo(channel: {
  ...
  // 4.5 👑 IMPERIAL COUNCIL ANALYSIS - Let philosophers evaluate the video
  console.log(`👑 Submitting to Imperial Council for approval...`);
  const councilAnalysis = await analyzeVideoByCouncil({
    id: videoId,
    title: bestStory.title,
    script: generatedScript.script,
    category: niche
  ...
  console.log(`📊 Council Verdict: ${councilAnalysis.finalVerdict}`);
  console.log(`⭐ Overall Score: ${councilAnalysis.overallScore}/100`);
  console.log(`🎯 Viral Potential: ${councilAnalysis.algorithmPrediction.viralPotential.toFixed(1)}/100`);

  // If not approved by council, revise the video
  if (!councilAnalysis.approvedForPublishing) {
    console.log(`⚠️ Video rejected by council. Required changes:`, councilAnalysis.requiredChanges);
    // Auto-revise title based on council feedback
    let revisedTitle = bestStory.title;
    if (councilAnalysis.requiredChanges && councilAnalysis.requiredChanges.length > 0) {
      // Apply Machiavelli's power words if needed
      if (!revisedTitle.match(/(Secret|Hidden|Truth|Exposed)/i)) {
        revisedTitle = `The Hidden Truth About ${revisedTitle.replace(/^(The |A |An )/i, '')}`;
      }
      console.log(`✨ Revised title: ${revisedTitle}`);
    }
    
    // Re-analyze with revised title
    const revisedAnalysis = await analyzeVideoByCouncil({
      id: videoId,
      title: revisedTitle,
      script: generatedScript.script,
      category: niche
    ...
    bestStory.title = revisedTitle;
    councilAnalysis.overallScore = revisedAnalysis.overallScore;
    councilAnalysis.approvedForPublishing = revisedAnalysis.approvedForPublishing;
    councilAnalysis.finalVerdict = revisedAnalysis.finalVerdict;
  }

  // 5. Save to channel's video history (only if approved)
  ...
  videos.push({
    ...
    estimatedViews: councilAnalysis.algorithmPrediction.estimatedViews,
    estimatedWatchTime: councilAnalysis.algorithmPrediction.estimatedWatchTime,
    duration: 720,
    isFirstVideo: true,
    autoGenerated: true,
    councilAnalysis: councilAnalysis,
    approvedByCouncil: councilAnalysis.approvedForPublishing,
    priority: councilAnalysis.overallScore >= 85 ? 'critical' : 
              councilAnalysis.overallScore >= 70 ? 'high' : 'medium'
  });
  localStorage.setItem(`videos_${channel.id}`, JSON.stringify(videos));

  console.log(`✅ First video generated and approved by council: ${bestStory.title}`);
  console.log(`👑 Council approval: ${councilAnalysis.approvedForPublishing ? 'YES' : 'CONDITIONAL'}`);
...
export async function autoplanVideosUntilMonetization(
  ...
  for (let i = 0; i < stories.length; i++) {
    ...
    // Generate script for council analysis
    const aiStory: Story = {
      id: story.id,
      title: story.title,
      category: story.category,
      origin: niche,
      country: story.location,
      language: 'English',
      era: 'Modern',
    ...
    // 👑 Submit to Imperial Council for approval
    console.log(`👑 [${i + 1}/${stories.length}] Submitting "${story.title}" to council...`);
    const councilAnalysis = await analyzeVideoByCouncil({
    ...
    // Auto-revise if rejected
    let finalTitle = story.title;
    let finalAnalysis = councilAnalysis;
    
    if (!councilAnalysis.approvedForPublishing) {
      console.log(`⚠️ Council rejected. Auto-revising...`);
      // Apply power words
      if (!finalTitle.match(/(Secret|Hidden|Truth|Exposed|Revealed)/i)) {
        finalTitle = `${councilAnalysis.overallScore >= 60 ? 'The Truth About' : 'Hidden Secrets of'} ${finalTitle.replace(/^(The |A |An )/i, '')}`;
      }
      
      // Re-analyze
      finalAnalysis = await analyzeVideoByCouncil({
        id: `planned-${Date.now()}-${i}`,
        title: finalTitle,
        script: generatedScript.script,
        category: story.category
      ...
      console.log(`✨ Revised to: "${finalTitle}" (Score: ${finalAnalysis.overallScore}/100)`);
    } else {
      console.log(`✅ Approved! Score: ${councilAnalysis.overallScore}/100`);
    }

    // Determine priority based on council score
    let priority: PlannedVideo['priority'] = 'medium';
    if (finalAnalysis.overallScore >= 85 || i < 3) priority = 'critical';
    else if (finalAnalysis.overallScore >= 75 || i < 7) priority = 'high';
    else if (finalAnalysis.overallScore < 60) priority = 'low';
    ...
    videoQueue.push({
      ...
      estimatedViews: finalAnalysis.algorithmPrediction.estimatedViews,
      estimatedWatchTime: finalAnalysis.algorithmPrediction.estimatedWatchTime,
      priority,
      status: 'ready', // Changed from 'planned' to 'ready' since script is generated
      councilAnalysis: finalAnalysis,
      approvedByCouncil: finalAnalysis.approvedForPublishing
    });
  }

  // Sort by priority and council score
  videoQueue.sort((a, b) => {
    const priorityOrder = { critical: 0, high: 1, medium: 2, low: 3 };
    const aPriority = priorityOrder[a.priority];
    const bPriority = priorityOrder[b.priority];
    if (aPriority !== bPriority) return aPriority - bPriority;
    return (b.councilAnalysis?.overallScore || 0) - (a.councilAnalysis?.overallScore || 0);
  });
